...
$app->post('/login', function() use ($app) {
    ...
    $userinfo = $db->getOneRecord("select uid,name,password,email,created,registered from users_auth where name='$username'");
    $userPermission = $userinfo['registered'];

    if($userPermission == "false") {
        $response['status'] = "error";
        $response['message'] = 'Login failed. Your account has not been verified';
    }
...
$app->post('/signUp', function() use ($app) {
    ...
    if(!$isUserExists){
        ...
        if ($result != NULL) {
            $response["status"] = "success";
            $response["message"] = "Apply for account successfully, please wait administrator to check your info.";
